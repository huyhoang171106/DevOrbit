...
- `cd devorbit-web; rtk npm run preview`: Vite preview server for production build.
- `cd devorbit-web; rtk npm run doctor`: run React Doctor health check (package.json script). No dedicated lint npm script exists — use `npx eslint .` for ESLint checks.
