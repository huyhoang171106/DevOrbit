 ...
 *
 * Contains the JWT access token, refresh token, plus basic student profile fields.
 * The access token must be sent as "Authorization: Bearer <token>" in subsequent requests.
 * When the access token expires, use the refresh token at POST /api/auth/refresh.
 ...
 */
public record StudentAuthResponse(String token, Long id, String studentCode, String fullName, String email, String avatar, String refreshToken) {}
