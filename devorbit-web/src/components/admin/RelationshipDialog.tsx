    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
      <div className="glass-card w-full max-w-lg p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold font-heading text-clay-text">Thêm quan hệ</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div>
            ...
            <select name="relatedCourseId" value={form.relatedCourseId} onChange={handleChange} className="input-field" required>
              <option value={0}>Chọn môn liên quan...</option>
              {courses.map((c) => (
                <option key={c.id} value={c.id}>{c.code} - {c.name}</option>
              ))}
            </select>
          ...
          {form.courseId !== 0 && form.relatedCourseId !== 0 && form.courseId === form.relatedCourseId && (
            <p className="text-xs text-rose-400">Một môn không thể liên quan đến chính nó</p>
          )}
