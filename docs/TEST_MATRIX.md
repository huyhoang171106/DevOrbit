...
| --- | --- | --- | --- | --- | --- | --- | --- |
| AUTH-001 | Student/admin auth, profile, password reset, protected AI roadmap, bookmarks, and Supabase Data API hardening | pass | pass | smoke | PostgreSQL catalog + Maven + Vitest + Vite build + run.bat startup | implemented | 2026-06-17 user-domain audit fixed reset contract drift, OTP resend malformed input, normalized duplicate registration, concurrent duplicate races, disabled/deleted admin stale tokens, password-reset stale student sessions via `token_version`, unknown JWT types, profile loading/upload auth handling, protected AI roadmap auth, bookmark duplicate normalization, safe storage filenames, and backend-owned Data API grants. Catalog proof: `student_users.token_version` present with nonnegative constraint, 0 backend-owned grants to `anon`/`authenticated`, 0 public tables without RLS, 0 missing FK indexes, 0 duplicate student keys, 0 dangling non-null chat student references. Tests/proof: backend focused auth/storage/Supabase tests passed, full backend `.\mvnw.cmd test -B` passed 218/218, `.\mvnw.cmd compile -B` passed, frontend `npm test -- --run --maxWorkers=1` passed 167/167, `npx tsc --noEmit` passed, `npm run build` passed, `.github/workflows/ci.yml` parsed via PyYAML because `yq` was unavailable, and `devorbit-api/run.bat` started successfully with `/api/courses` 200 and `/api/student/me` without token 401. |
...
| US-040 | Mobile Student Auth Registration Session | pass | no | no | Gradle unit test + debug build | implemented | `devorbit-mobile`: targeted `AuthSessionPolicyTest`, `AuthValidationTest`, `AuthErrorMessageParserTest`, and `AuthStateTransitionsTest` passed; full `.\gradlew.bat :app:testDebugUnitTest` passed; `.\gradlew.bat :app:assembleDebug` passed; `installDebug` blocked because `adb devices` returned no connected devices |
| US-041 | Mobile Repos By Course and GPA Calculator Tabs | pass | no | no | Gradle unit test + debug build | implemented | `devorbit-mobile`: targeted nav/course catalog/repo DTO/GPA tests covered Vietnamese nav labels without Explore, repo-backed course filtering, null-safe repo metadata, row metadata, GPA logic, and GPA state; full `.\gradlew.bat :app:testDebugUnitTest` passed; `.\gradlew.bat :app:assembleDebug` passed; emulator smoke for this update was not run because `adb devices` returned no connected devices |
