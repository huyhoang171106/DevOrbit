...
| US-041 | Mobile Repos By Course and GPA Calculator Tabs | pass | no | no | Gradle unit test + debug build | implemented | `devorbit-mobile`: targeted `ScreenTest`, `ReposCourseRowModelTest`, `GpaCalculatorTest`, and `GpaScreenStateTest` passed; full `.\gradlew.bat :app:testDebugUnitTest` passed; `.\gradlew.bat :app:assembleDebug` passed |
