...
| US-041 | Mobile Repos By Course and GPA Calculator Tabs | pass | no | no | Gradle unit test + debug build + emulator smoke | implemented | `devorbit-mobile`: targeted nav/course catalog/GPA tests covered Vietnamese nav labels, repo-backed course filtering, row metadata, GPA logic, and GPA state; full `.\gradlew.bat :app:testDebugUnitTest` passed; `.\gradlew.bat :app:assembleDebug` passed; `.\gradlew.bat :app:installDebug` passed; emulator showed `MainActivity` focused with the `Môn học` tab listing repo-backed courses sorted by repo count |
