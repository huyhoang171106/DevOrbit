...
| US-020 | GPA Calculator + Integer Credits + Semester Presets + Preset Stats/Merge + Cumulative Estimate + Goal Planner + Goal What-If + Row UX + Explicit Browser Draft Controls + Background Atmosphere | pass | no | no | Vite build | implemented | `devorbit-web`: `npm test -- GpaCalculatorPage`; `npm test -- GpaCalculatorPage router`; `npm run build` |
