...
- Tables: none.
- Domain rules: client-side calculator using weighted 10-point average by credits; cumulative estimate uses `(current GPA * completed credits + semester GPA * semester credits) / (completed credits + semester credits)`; semester presets include only courses with the effective semester matching the selected term and `credits > 0`. Effective semester comes from the saved learning roadmap map when present, otherwise from the course catalogue.
...
| --- | --- |
| Unit | Vitest render tests for weighted 10-point GPA calculation, row add/remove, validation guidance, no 4-point output, semester preset loading, saved learning roadmap assignment precedence, cumulative GPA estimate, and route rendering |
